                ...
                if(i > fday && i <= (aday + fday)) {
                    ...
                    if(iday === curDate.getDate() && ayear === curDate.getFullYear() && amonth == curDate.getMonth()) {
                        ...
                        if(dateArea.length && dateArea.indexOf(curday) == -1) {
                            tds.push('<td class="today disabled" data-date="'+curday +'" id="'+curday+'">今天</td>');
                        } else {
                            tds.push('<td class="today" data-date="'+curday +'" id="'+curday+'">今天</td>');
                        }
                        // 过去的日期
                    } else if(dateExtend.compare(curDate, curday) > 0 || (dateArea.length && dateArea.indexOf(curday) == -1)) {
                        tds.push('<td class="disabled" data-date="'+curday +'" id="'+curday+'">'+ iday +'</td>');
                    } else {
                        // 是否设置了可选日期
                        tds.push('<td data-date="'+curday+'" id="'+curday+'">'+ iday +'</td>');
                    }
                } else if(!stop){
                    tds.push('<td></td>');
                }
        ...
        _bindEvent: function() {
            ...
            tds.off('click').on('click', function(e) {
                ...
                if(type == 'single') {
                    ...
                    var time_str = time.getTime().toString();
                    var checkin=time_str.substr(0, 10)
                    var checkOut=time_str.substr(0, 10)
                    console.debug(checkin,checkOut)*/
                    /*alert(dateArea)*/

                    // 最多可选择多少天
                    if(selected.length >= 2 || (selected.length == 1 && (days > dateArea || days < 0))) {
                        // 去掉提示语
                        el.find('.selected').find('span').remove();
    ...
    $(".yes").click(function(event) {
        if(pageData.length==2){
         $("#mask").hide()
            ...
            for(i=pageData.length-2;i<pageData.length;i++){
                  pa.push(pageData[i]);         
            }
           // alert(pa) 
           $("#mask").hide()
