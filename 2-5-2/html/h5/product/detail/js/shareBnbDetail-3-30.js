...
//内容请求
$(document).ready(function () {
    $(".navIcon span").css({"display":"inline-block","margin-top":"0.26rem","vertical-align":"middle"});
// alert($(".navDate").offset().top)
    //ifLogin();
    if(getRequest().couponId!=undefined){
        if(getRequest().couponId=="null"){
            var reqData={"id":id,"assetType":getRequest().assetType};
        }else{
                ...
                for (var k = 0; k < roomList.length; k++) {
                    if(roomList[k].productInfo.productPrice!=undefined){
                        var roomshowCyType=roomList[k].productInfo.productPrice.showCyType,//类型
                            ...
                            if(roomshowCyType==0||roomshowCyType==5){
                    ...
                    if (roomList[k].productInfo && roomList[k].productInfo.productType != 0) {
                        ...
                        if(roomshowCyType==0){
                            ...
                            $(".type").eq(k).find(".type_price .type_number").css({"font-size":"0.867rem","line-height":"1.6rem"});
                        }

                        if (roomList[k].productInfo.tolAmount == 0) {
                            $(".type").eq(k).children(".type_price").css("color","#999").children("strong").addClass("gray").html("满房");
            ...
            if(color!="999"){//表示点的不是已过去的日期
                ...
                if(firstS==1){//入住
                    if(cancel==0){
                        ...
                        $("#calendar table td").css({"background-image":"none","background-color":"#fff"}).find("span").remove();
                        $(this).css({"background-image":"url(http://7xio74.com2.z0.glb.clouddn.com/ru.png)","background-color":"#fff","color":"#fff"}).append("<span>入住</span>");
                ...
                if(firstS==2){//离店
                    $(this).css({"background-image":"url(http://7xio74.com1.z0.glb.clouddn.com/li.png)","background-color":"#cfe5e5","color":"#fff"}).append("<span>离店</span>");
                    ...
                    if(loop==1){
                        $("#calendar table").eq(begin).find("td").each(function(){
                            ...
                            if($(this).css("background-image")=="url(http://7xio74.com2.z0.glb.clouddn.com/ru.png)"||$(this).css("background-image")=='url("http://7xio74.com2.z0.glb.clouddn.com/ru.png")'){
                                $(this).css({"background-image":"url(http://7xio74.com2.z0.glb.clouddn.com/ru.png)","background-color":"#cfe5e5"});
                            }
