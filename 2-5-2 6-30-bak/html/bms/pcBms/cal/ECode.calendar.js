        ...
        ECode.calendar = function(options){
            ...
            var calendarfn = {
                ...
                createContainer:function(){//创建日历最外层div
                    ...
                    container.css({"position":"absolute","float":"left","zIndex":1001,"left":inputPos.left-def.pos.right,"top":inputPos.top+input.outerHeight()+def.pos.top});
                ...
                drawDate:function(oCal,odate){//绘制日历
                    var dateWarp, titleDate, dd, year, month, date, days, weekStart,i,l,ddHtml=[],arr=[],oA,sV,iCur,weeks,aDate,oDate;
                ...
                dragTime:function(id,diff){//时间轴功能
                    var today=new Date(),h=today.getHours(),m=today.getMinutes(),that=this;
                    var _id = $(id);
                    var unitH;
                    var diffs = diff;
                    ...
                    links.each(function(a){
                        $(this).click(function(){
                            ...
                            if(showbox==null){
                                ...
                                input[1].tagName.toUpperCase()==="INPUT"?input.val($(this)[0]["data-date"]+" "+(isWeek?weeks:(isTime?hour+':'+min:''))):input.text($(this)[0]["data-date"]+" "+(isWeek?weeks:(isTime?hour+':'+min:'')))
                            }else{
                            ...
                            enddate=$(this)[1]["data-date"]; 
                            starttime = [hour,min];
                            alert(startdate)
                            // isTime?$(this).addClass("on").siblings().removeClass("on"):that.removeDate();
                            if(callfn){
                                callfn();
                            }
                        })
