...
$(document).ready(function () {
    ...
    //     }
    // }
    getDetail(".card", cardCodeArr);
    getDetail(".coupon", couponCodeArr);
...
function active(dom) {
    $(dom).click(function () {
        ...
        tabName == "coupons" && $("." + tabName).length > 5 ? $(".checkmore").css("visibility", "visible") : $(".checkmore").css("visibility", "hidden");
...
function getDetail(dom, arr) {
    $(dom).click(function () {
        ...
        window.location.href = "/html/h5/member/giftcard/cardOrCoupons.html?couponCode=" + arr[domIndex];
    })
}
//获取url的参数
function getRequest() {
...
function getList(type, hotelIds) {//获取列表信息
    if ("cards" == type) {//会员卡列表
        ...
        $.ajax({//商户详情接口
            ...
            data: { data: JSON.stringify({ hotelIds: hotelIds, pageno: 1, pagecnt: 50 }) },
            success: function (data) {
                console.log(data);
                ...
                if (undefined != data.data) {
                    $.each(data.data, function (i) {
                        ...
                        var iconStatus = {};
                        if ("1" === tradable) {//是否可转让
                            iconStatus.transfer = { "status": 1, "url": transferUrl };
                        } else {
                            iconStatus.transfer = { "status": 0, "url": null };
                        }
                        ...
                        if ("1" === businessExchange) {//是否可换宿换商品
                            iconStatus.hotels = { "status": 1, "url": exchangeUrl };
                            iconStatus.goods = { "status": 1, "url": goodspurchaseUrl };
                        } else {
                            iconStatus.hotels = { "status": 0, "url": null };
                            iconStatus.goods = { "status": 0, "url": null };
                        }
