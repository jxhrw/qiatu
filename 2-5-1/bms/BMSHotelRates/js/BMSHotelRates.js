...
//房价信息
    ...
    gainData(nowDate);
//悬停于列表中
    $(".housePrice").on("mouseover",".canselected",function () {
        // console.log(111);
...
//单击选框修改房价
    $("body").on("click",".move",function () {
        ...
        var houseType = parseInt($(this).parent().attr("serail"))-1;
        ...
        var currentDate = {"checkin":houseStart,"checkout":houseEnd}
            $.ajax({
                ...
                success:function (priceData) {
                    console.log("success");
                    roompid = priceData.data[houseType].roomPid;
                    roomCheckin = Date.parse(new Date(priceData.data[houseType].roomStatus[first].checkin));
                    roomCheckout = Date.parse(new Date(priceData.data[houseType].roomStatus[last].checkin));
  ...
  //跟换日期
    laydate({
        ...
        choose: function(datas){ //选择日期完毕的回调
            var Y = getFutureDate(datas).y;
            var M = getFutureDate(datas).m;
            var D = getFutureDate(datas).d;
            var date = Y+"-"+M+"-"+D;
    ...
    $(".J-batchSave").click(function () {
        ...
        roomCheckout = ((roomCheckin/1000) + 60*60*24*14)*1000;
        var weekDays = [];
