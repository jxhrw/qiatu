            ...
            $.ajax({
   	    	...
   	    	success:function (data) {
   	    			...
   	    			$("body").on("click",".J-OFFbtn",function () {
   	    				console.log("ffff");
				   	    $(".active").children(":last-child").attr("class","J-off");
				   	    		$(".active").children(":last-child").html("已下线");
				   	    		// console.log("success");
				   	    		close();
				   	})
    		...
    		$(".up-warp").hide();
   	    	...
   	    	success:function (data) {
   	    			...
   	    			$("body").on("click",".J-Upbtn",function () {
				   	    $(".active").children(":last-child").attr("class","J-ing");
				   	    		$(".active").children(":last-child").html("进行中");
				   	    		// console.log("success");
				   	    		close();
				   	})
...
// 填充数据
    function Tian(boj,i,data) {
		...
		if (Status == "0") {
			...
			$(boj).children(":last-child").append("即将开奖");
...
function TimeG(time) {
  ...
  //   var rl = "http://test.jiheclub.com/html/bms/createActivity/index.html";
  // 	console.log(2);
  // 	$("body").on("click",".J-setact",function (rl) {
  // 		console.log(1);
  // 		if ( document.location.search == "") {
  // 			console.log(1);

  // 			newUrl = rl +　"?actType=" + acttype;
  // 		} else {
  //           newUrl = changeURLArg(rl,actType,acttype);
  // 		}
