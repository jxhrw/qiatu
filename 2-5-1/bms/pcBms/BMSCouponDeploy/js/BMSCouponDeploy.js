        ...
        $.ajax({
        	...
        	success:function (Mtedata) {
                ...
                if (Mtedata.data.faceValue == undefined) {
	          	...
	          	if (!("useConditionList" in Mtedata.data)) {
	          		return 0;
	          	} else {
	          		for (var i=0;i<Mtedata.data.useConditionList.length;i++) {
	                	if (Mtedata.data.useConditionList[i].conditionType == "cond-date") {
	                		// console.log(datetxt);
	                		if ( !("dateUsable" in Mtedata.data)) {
	                        	$(".usagedate").hide();
	                        } else {
	                			$(".usagedate").show();
		                		...
		                		$(".usagedate").html("除“"+datetxt+"”以外均可使用");
	                        }

	                	
	                	} else if (Mtedata.data.useConditionList[i].conditionType == "cond-aheadoftime") {
	                		    $(".yhj_concent:eq(7)").show();
          ...
          if (couponid == undefined) {
          	...
          	$(".cha_yhj").show();
          	$(".tcback").show();
          	var effectivet = getDate(parseInt(Listdata.data[order].effectiveTime));
    		var expiret = getDate(parseInt(Listdata.data[order].expireTime));
...
//房卷 优惠券 消费金切换
    $(".house").click(function () {
    	...
    	$(".concent").remove();
    	$(".new-btn").show();
    	$(".new-btn").children().html("新建房券");
    	$(".new-btn").children().attr("coupontype","1");
    	$(".list-title").children("td:eq(3)").html("流通量(张)");
    	$(".list-title").children("td:eq(4)").html("核销量(次)");
    ...
    $(".pay").click(function () { 
    	...
    	$(".new-btn").hide();
    	$(".list-title").children("td:eq(3)").html("流通量(元)");
    	$(".list-title").children("td:eq(4)").html("核销量(元)");
    ...
    $(".favotable").click(function () {
    	...
    	$(".concent").remove();
    	$(".new-btn").show();
    	$(".new-btn").children().html("新建优惠券");
    	$(".new-btn").children().attr("coupontype","5");
    	$(".list-title").children("td:eq(3)").html("流通量(张)");
    	$(".list-title").children("td:eq(4)").html("核销量(次)");
    	$(".wu").children(":last-child").html("新建优惠券");
    	$.each(Listdata.data,function (index) {
...
//填充数据
    ...
    function TianSj(obj,i,ztdata) {
        ...
        $(obj).children("td:eq(3)").append(ztdata.data[i].circulation);
    ...
    function TianZt(obj,i,ztdata) {
    	if (ztdata.data[i].publishAmount == undefined) {
    		...
    		$(obj).children("td:eq(4)").html(ztdata.data[i].publishAmount);
    	}
    	...
    	if (ztdata.data[0].publishAmount == 0) {
    		...
    		if (timestamp <= parseInt(ztdata.data[i].expireTime)){
            	...
            	$(obj).children("td:eq(8)").append("已结束");
