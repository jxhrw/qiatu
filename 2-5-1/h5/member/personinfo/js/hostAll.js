//url处理
var url = location.href;
host='http://'+window.location.host;
root=window.location.pathname;
h=$(window).height();
/*alert(root);*/
function getParameter(paraStr, url)
{
    var result = "";
    //获取URL中全部参数列表数据
    var str = "&" + url.split("?")[1];
    /*var stri=url.split("?")[1];
    alert(stri);*/
    var paraName = paraStr + "=";
    //判断要获取的参数是否存在
    if(str.indexOf("&"+paraName)!=-1)
    {
        //如果要获取的参数到结尾是否还包含“&”
        if(str.substring(str.indexOf(paraName),str.length).indexOf("&")!=-1)
        {
            //得到要获取的参数到结尾的字符串
            var TmpStr=str.substring(str.indexOf(paraName),str.length);
            //截取从参数开始到最近的“&”出现位置间的字符
            result=TmpStr.substr(TmpStr.indexOf(paraName),TmpStr.indexOf("&")-TmpStr.indexOf(paraName));
        }
        else
        {
            result=str.substring(str.indexOf(paraName),str.length);
        }
    }
    else
    {
        result="无此参数";
    }
    return (result.replace("&",""));
}
var r = getParameter("member_hotelid",url);
id =r.substring(r.lastIndexOf('=')+1, r.length);
var reNum = /^\d+$/;
if(reNum.test(id) == false) {
   id="";
}
var r1 = getParameter("imgUrl",url);
imgUrl =r1.substring(r1.lastIndexOf('=')+1, r1.length);
var r2 = getParameter("title",url);
title =r2.substring(r2.lastIndexOf('=')+1, r2.length);
var r3 = getParameter("desc",url);
desc =r3.substring(r3.lastIndexOf('=')+1, r3.length);
var r4= getParameter("shareLink",url);
shareLink =r4.substring(r4.lastIndexOf('=')+1, r4.length);
var r5 = getParameter("detailLink",url);
detailLink =r5.substring(r5.lastIndexOf('=')+1, r5.length);
stri=url.split("?")[1];
/*$(document).ready(function() {
    document.title="几何试睡－杭州法云安缦"
});*/
var r6 = getParameter("obj",url);
obj =r6.substring(r6.lastIndexOf('=')+1, r6.length);
var r7 = getParameter("regsucc_tourl",url);
regsucc_tourl=r7.substring(r7.lastIndexOf('=')+1, r7.length);
var r8 = getParameter("orderid",url);
orderid =r8.substring(r8.lastIndexOf('=')+1, r8.length);


