$(document).ready(function(){
            ...
            if(res.sc==0){
                ...
                if(res.data.existTradePasswd=="1"){
                    ...
                    if(currencyType==1){
                        ...
                        $("#goldFees").html(parseInt(shouldPrice*feesProportion/100)+"积分");
                    }
        ...
        $.post(h5orClient(url),{data:JSON.stringify(data)},function(resu){
            ...
            if(resu.sc==0){
                ...
                if(ua.match(/MicroMessenger/i) != 'micromessenger' && /(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) && /jihe/i.test(navigator.userAgent)){
                    window.location.href="/html/h5/market/release.html?app=jiheios&sellingid="+resu.data;
                }
                else {
                    window.location.href="/html/h5/market/release.html?sellingid="+resu.data;
                }
            }
        ...
        $("#sure").click(function(){
            ...
            $.post(h5orClient(url),{data:JSON.stringify(data)},function(resu){
                ...
                if(resu.sc=="0"){
                    ...
                    if(ua.match(/MicroMessenger/i) == 'micromessenger'){
                        window.location.href="/html/h5/member/personinfo/password.html?type="+type+"&verifycode="+verifycode;//这是微信浏览器
                    }else{
                        if(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)){
                            window.location.href="/html/h5/member/personinfo/password.html?type="+type+"&verifycode="+verifycode;//这是iOS平台下浏览器
                            if(/jihe/i.test(navigator.userAgent)){
                                window.location.href="/html/h5/member/personinfo/password.html?app=jiheios&type="+type+"&verifycode="+verifycode;//这是iOS平台下app
                            }
                        }
                        if(/android/i.test(navigator.userAgent)){
                            window.location.href="/html/h5/member/personinfo/password.html?type="+type+"&verifycode="+verifycode;//这是Android平台下浏览器
                        }
                    }
                }else {
    ...
    function calculatedPrice(){
        ...
        if(currencyType==0){
            ...
            $("#shouldPrice").html(Math.ceil(numDisc*(saleMon).toFixed(1)));
                ...
                if(undefined!=JSON.parse(sessionStorage.getItem("isHavePasswd"))){
                    ...
                    if(isIt.existTradePasswd==0 && ( undefined!=JSON.parse(sessionStorage.getItem("tradeToken"))&& ""!=JSON.parse(sessionStorage.getItem("tradeToken")) ) ){
                        ...
                        if(currencyType==1){
                            ...
                            $("#goldFees").html(parseInt(shouldPrice*feesProportion)+"积分");
                        }
                        $(".popupT,#confirmationAmount").show();
                    }
                }
