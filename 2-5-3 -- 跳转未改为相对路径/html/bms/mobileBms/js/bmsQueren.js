        ...
        if(data.sc==0){
            ...
            for (var i =0; i < data.data.settlePayments.length; i++) {
                ...
                if(data.data.settlePayments[i].payType==1){//房券
                    ...
                    testArr.push([data.data.settlePayments[i].couponCode,data.data.settlePayments[i].couponName,data.data.settlePayments[i].amount/100,data.data.settlePayments[i].couponRemark,data.data.settlePayments[i].faceValue]);
...
function getDate(nS) {
    ...
    return Beginy+"/"+Beginm+"/"+Begind +" &nbsp;&nbsp;&nbsp;&nbsp; "+formatNum(hour)+":"+ formatNum(minute) +":"+ formatNum(second);
