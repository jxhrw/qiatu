...
function handleData(resp){
    ...
    if(resp.data.allowCancel && resp.data.allowCancel=="1"){
        $(".cancellation").show().css("display","block");
    }else {
        $(".cancellation").hide();
    }
    ...
    $('.seeCustomerName').text(resp.data.customerName+"（"+memberGrade+"）");
    ...
    if("9" == resp.data.status){
        var bookResult='';
        if(resp.data.bookResult){
        ...
        }else if(resp.data.closereson){
            bookResult=resp.data.closereson;
        }
...
$("#download").click(function () {
    ...
    delete searchData.pagecnt;
    window.location.href = '/order/bmsh5/export?data='+ JSON.stringify(searchData);
});
