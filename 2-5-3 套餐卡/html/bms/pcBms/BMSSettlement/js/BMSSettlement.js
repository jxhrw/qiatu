$(document).ready(function(){
    ...
    $("#cashRecords").click(function(){
        window.location.href='BMSPayBalance.html?accountId='+accountId+'&accountType='+accountType1+'&status=1';
    });
    //现金结算待结算记录
    $("#cashUnRecords").click(function(){
        window.location.href='BMSPayBalance.html?accountId='+accountId+'&accountType='+accountType1+'&status=0';
    });
    //积分中心结算收支明细
    $("#integralRecords").click(function(){
        window.location.href='BMSPayBalance.html?accountId='+accountId+'&accountType='+accountType2+'&status=1';
    });
    //积分中心结算待结算记录
    $("#integralUnRecords").click(function(){
        window.location.href='BMSPayBalance.html?accountId='+accountId+'&accountType='+accountType2+'&status=0';
    });


    //充值提现记录,页面a链接
    $(".table .aBtn,#cashRechargew,#integralRechargew").click(function(){
        window.location.href='BMSRechargeDraw.html?accountId='+accountId;
    });
    ...
    if(timeNow>=2016){
        ...
        $(".selectClass").html(optionHtml);
        //年度收支明细
        $("#cashYear").click(function(){
            if(undefined!=$(this).val() && ""!=$(this).val()){
                window.location.href='BMSPayBalance.html?accountId='+accountId+'&accountType='+accountType1+'&status=-1'+'&years='+$(this).val();
            }
        });
        $("#integralYear").click(function(){
            if(undefined!=$(this).val() && ""!=$(this).val()){
                window.location.href='BMSPayBalance.html?accountId='+accountId+'&accountType='+accountType2+'&status=-1'+'&years='+$(this).val();
            }
        });
    }else {
        $(".selectClass").hide();
        ...
        $.ajax({
            ...
            success:function(res) {
                ...
                if(res.sc==0){
                    ...
                    if(noList!=0){
                        var theadHtml="<tr><th>申请时间</th><th>充值/提现银行帐号|流水号</th><th>账户类型</th><th>充值金额(元)</th><th>提现金额(元)</th><th>申请人</th><th>状态</th><th>处理完成时间</th><th>备注</th><th>操作</th></tr>";
                        var tbodyHtml="";
                        for(var i=0;undefined!=transferInfoList && i<transferInfoList.length;i++){
                            ...
                            if(operation=='删除'){
                                operation='<span style="cursor: pointer" class="delete" id="'+ transferInfoList[i].id +'">'+operation+'</span>';
                            } else {
                                operation='';
                            }
    ...
    //充值提现记录删除，数据请求
    $(".table tbody .delete").live("click",function(){
        ...
        $.ajax({
            ...
            dataType:'json',
            success:function(res) {
                $(".table tbody .delete#"+id).parents("tr").remove();
                request(0);
            }
        });
    });
