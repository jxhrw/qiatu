...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success:function(res) {
                ...
                if(res.sc==0){
                    ...
                    $("#summary").html("总收入："+settledAmount/100+"元 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+"已结算："+transSettlementAmount/100+"元 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+"待结算："+willSettledAmount/100+"元 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='"+loadUrl+"' download='"+GetParams().years+"年度结算明细' class='button' id='download'>"+GetParams().years+"年度结算明细excel下载</a>");
                }
                else {
                    console.log(res.sc);
                }
    ...
    $("#searchDown").click(function () {
        ...
        delete b.pageno;
        delete b.pagecnt;
        window.location.href='/accounting/bms/excel/detail?data='+JSON.stringify(b);
    });

    //起止时间的3个选择
    $("body").on('click',"#timeChoose em",function(event){
    ...
    //详情
    $("body").on('click',"#tableCon em", function(event) {
        var id=$(this).attr("id");
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            if(res.sc=="0"){
                ...
                var tbodyHtml="<thead><tr><th>结算时间</th><th>交易/订单号</th><th>收入/支出（元）</th><th>余额（元）</th><th>状态</th><th>操作</th></tr></thead><tbody>";
                for(var i=0;undefined!=transferInfoList && i<transferInfoList.length;i++){
                    ...
                    if(undefined!=transferInfoList[i].orderId){
                        ...
                        subBtn='<em type="1" class="operation" id="'+ transferInfoList[i].orderId +'">详情</em>';
                    }else{
                        orderId='';
                        subBtn='';
                    }
                    if("11311"==transferInfoList[i].accountingItem){
                        subBtn='<em type="2" class="operation" id="'+ transferInfoList[i].pointsRecordId +'">详情</em>';
                    }
                ...
                $("#tableCon").html(tbodyHtml+'</tbody>');

                //页数
                if(res.data.totalPages*1>=1){
                    $('#pageDir').createPage({
                        pageCount:parseInt(res.data.totalPages),
