$(document).ready(function() {
	...
	$.post('/bms/h5/business/is_login.json',  function(data) {
		console.log(data);
		if(data.sc==0){
			if(data.data.initPwd == "1"){ //强制修改密码,是否初始密码0-否 1-是
				$("#firstChange").show();
				$(".maskClose").hide();
				$(".mask").show();
			}else {
	...
	$(".dl2 dd input").blur(function (event) {
		var newPassword = $(this).val();
		var reg = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,100}$/;//必须包含数字和字母，>=6位
						...
						(function(v){
							...
							p.click(function(event) {
								...
								if ($(this).attr("class") == "hasHide hasHideOpen") {
									$(".BMSRightBottom").attr('src',submenu[0].privilegeValue);
									$('.afterClick').removeClass('afterClick');
									$(this).next(".divHide").find(".pHide").eq(0).parent().addClass('afterClick');
								}
							});
							for(i in submenu){
