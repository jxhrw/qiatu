...
//参与人数列表页
    $(".J-partbtn").click(function () {
    	$(".partLi").remove();
    	partData={"actId":actid}
    	$.ajax({
    		type:"POST",
    		typedata:"json",
    		url:"/activity/bms/latestcodes",
    		data:{data:JSON.stringify(partData)},
    		success:function (participationData) {
    			if (participationData.sc == "0") {

					var minuteData = "";
					for (var i=0;i<participationData.data.length;i++) {
            var partDate = new Date(parseInt(participationData.data[i].createTime));
            var partHours = partDate.getHours();
	    			var partMinutes = partDate.getMinutes();
						var gainDate = getDate(parseInt(participationData.data[i].createTime)) +
                " " + TimeG(partHours) + ":" + TimeG(partMinutes);
						console.log("success");
						minuteData += "<li class='partLi'><span class='ulPartFirst'>"+ participationData.data[i].nickname +"</span>"
   	    							 +"<span class='ulPartThrid'>"+ participationData.data[i].code +"</span>"
   	    							 +"<span class='ulPartFour'>"+ gainDate +"</span></li>"
					}
                    $(".partFirstLi").after(minuteData);

    			} else {
                    console.log(participationData.data.sc);
    			}
    		},
    		error:function () {
    			console.log("error");
...
//--------------------悬浮列表改变gou样式
    $("body").on("mouseover mouseout",".concent",function (event) {
    	 ...
    	 if(event.type == "mouseover"&&imgAttr == "img/gou1.jpg"){
    		$(this).children(".J-first").children().attr("src","img/gou3.jpg");
		 }else if(event.type == "mouseover"&&imgAttr == "img/gou2.jpg"){
		  	$(this).children(".J-first").children().attr("src","img/gou2.jpg");
		 }else if(event.type == "mouseout"&&imgAttr == "img/gou3.jpg"){
		  	$(this).children(".J-first").children().attr("src","img/gou1.jpg");
		 }else if(event.type == "mouseout"&&imgAttr == "img/gou2.jpg"){
		  	$(this).children(".J-first").children().attr("src","img/gou2.jpg");
		 }
    })
//----------------------------
...
function ListData(data) {
		...
		$.each(data.data,function (index) {
			$("tbody").append(
				"<tr class='concent one' onclick='gouxuan(this)' order="+index+">"
			   +"<td class='J-first'><img src='img/gou1.jpg'></td>"
			   +"<td class='concentTdOne'></td>"
			   +"<td class='concentTdTwo'></td>"
			   +"<td class='concentTdThree'></td>"
			   +"<td class='concentTdFour'><span class='start-time'></span>至<span class='end-time'></span></td>"
			   +"<td class='concentTdFive'></td>"
			   +"<td class='concentTdSix'></td>"
			   +"<td id='last'></td></tr>")
...
// 填充数据
    function Tian(boj,i,data) {
		...
		$(boj).children(".concentTdOne").append(data.data[i].actTitle);

		if (data.data[i].actType == "1") {
			$(boj).children(".concentTdTwo").append("单人多码");
		} else if (data.data[i].actType == "2") {
			$(boj).children(".concentTdTwo").append("双人配对");
		} else if (data.data[i].actType == "3") {
			$(boj).children(".concentTdTwo").append("一元购抽奖");
		}
		$(boj).children(".concentTdThree").append(data.data[i].usersCount);
		...
		$(boj).children(".concentTdFour").find("span:eq(0)").append(actbeigintime);
		$(boj).children(".concentTdFour").find("span:eq(1)").append(actendtime);
		$(boj).children(".concentTdFive").append(actpubtime);
		$(boj).children(".concentTdSix").append(data.data[i].codesCount);
