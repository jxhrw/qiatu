...
$(document).ready(function() {
	...
	$.post('/user/bmsh5/infobyaccount', {data: JSON.stringify(data)}, function(data) {
		...
		if(data.sc=="0"){
			//退卡弹窗加载会员信息-------------
			$(".cardVIP").html(data.data.realname);
			$(".cardPhone").html(data.data.mobileAccount.accountName);
			//------------------------------------------------
...
//submit
	$(".hovertable td strong").on('click', function(event) {
...
//删改
	$(".hovertable tr span").on('click', function(event) {
		if($(this).index()==1){
...
$(document).ready(function() {
	...
	$("body").on("click",".writeOffRecord",function(){
		pageNum=1;
	...
	$(".writeOffBtn").click(function(){
		...
		$.post("/pay/bms/coupon/checkoff/add",{data:JSON.stringify(data)},function(res){
			if(res.sc==0){
				$(".mask10").hide();
...
$(document).ready(function () {
	...
	var affirmInputData;
	var mobile;
	var returnCouponCode;
	var num;
	//退卡
	$(".backVip").click(function () {
		var returnCard = "";
		$.ajax({
			type:"POST",
			datatype:"json",
			data:{data:JSON.stringify(offData)},
			url:"/coupon/bms/cardBack/list",
			success:function (offre) {
				for (var i=0;i<offre.data.length;i++) {
		            returnCard += "<li><span>"+ offre.data[i].couponBaseInfo.couponName +"</span><button class='returnCardBtn' couponCode="+ offre.data[i].couponCode +">退卡</button></li>";
				}
				$(".clubCard").children().remove();
				$(".clubCard").append(returnCard);
			}
		});
		$(".returnCard").show();
		$(".tcback").show();
	...
	$(".returnCardtop img").click(function () {
		$(".returnCard").hide();
		$(".tcback").hide();
	})
	//确认退卡
	$("body").on("click",".returnCardBtn",function () {
        $(".returnCard").hide();
		$(".returnCardAffirm").show();
		$(".tcback").show();
		returnCardCode = {"couponCode":$(this).attr("couponcode")};
		returnCouponCode = $(this).attr("couponcode");
		$(".affirmInputCount").val("");
		$.ajax({
			type:"POST",
			datatype:"json",
			url: "/coupon/bms/cardBack/info",
			data:{data:JSON.stringify(returnCardCode)},
			success:function (returnCardData) {
				mobile = returnCardData.data.mobile;
				if (returnCardData.sc == "0") {
					console.log("success");
					$(".cardName").html(returnCardData.data.couponBaseInfo.couponName);
					if (returnCardData.data.memberProjectName != undefined) {
						$(".cardPlan").show();
						$(".cardPlanLeft").show();
                    	$(".cardPlan").html(returnCardData.data.memberProjectName);
					} else {
						$(".cardPlan").hide();
						$(".cardPlanLeft").hide();
					}
                    $(".cardStartDate").html(getDate(parseInt(returnCardData.data.effectiveTime)));
                    $(".cardEndDate").html(getDate(parseInt(returnCardData.data.expireTime)));
                    if (returnCardData.data.LastUseDate != undefined) {
                    	$(".lastUse").show();
                    	$(".lastUseLeft").show();
                    	$(".lastUse").html(returnCardData.data.LastUseDate);
                    } else {
                    	$(".lastUse").hide();
                    	$(".lastUseLeft").hide();
                    }
                    for (var i=0;i<returnCardData.data.subCouponList.length;i++) {
                    	if (returnCardData.data.subCouponList[i].couponBaseInfo.couponType == "2") {
                            num = i;
                    	}
                    }
                    if (returnCardData.data.subCouponList[num].recyclableRemain != undefined) {
                    	$(".differenceRecharge").html(parseInt(returnCardData.data.subCouponList[num].recyclableRemain)/100);
                    } else {
                    	$(".differenceRecharge").html(0);
                    }
                    if (returnCardData.data.subCouponList[num].recyclablePresent != undefined) {
                    	$(".differenceGive").html(parseInt(returnCardData.data.subCouponList[num].recyclablePresent)/100);
                    } else {
                    	$(".differenceGive").html(0);
                    }
                    $(".differenceAll").html(parseInt($(".differenceRecharge").html()) + parseInt($(".differenceGive").html()));
				} else {
					console.log(returnCardData.sc);
				}

			}
		});
    });
	$(".returnAffirmtop img").click(function () {
		$(".returnCardAffirm").hide();
		$(".tcback").hide();
	})
	$(".affirmBtn").click(function () {

		affirmInputData = {"mobile":mobile,
							"grantList": [
					          {
					          	"recyclableRemain":$(".differenceRecharge").html(),
					            "couponCode": returnCouponCode,
					            "faceValue":parseInt($(".affirmInputCount").val())*100,//退卡金额
					           }
					       	]};
	    console.log(affirmInputData);
		if (parseInt($(".affirmInputCount").val()) <= parseInt($(".differenceRecharge").html()) && parseInt($(".affirmInputCount").val()) >=0) {
			$.ajax({
				type:"POST",
				datatype:"json",
				url: "/coupon/bms/cardBack/doOperate",
				data:{data:JSON.stringify(affirmInputData)},
				success:function (affirmData) {
					if (affirmData.sc == "0") {
	                   console.log("success");
	                   $(".returnCardAffirm").hide();
					   $(".tcback").hide();
					} else {
	                   console.log(affirmData.sc);
					}
				}
			});
		} else if (parseInt($(".affirmInputCount").val()) < 0) {
			jiHeAlert.open("退款金额不能小于0");
			$(".returnCardAffirm").show();
			$(".tcback").show();
		} else {
			jiHeAlert.open("退款金额不能大于充值金额");
			$(".returnCardAffirm").show();
			$(".tcback").show();
		}
	...
	function recordPage(couponCodeThis,userid){
		...
		$.post("/pay/bms/coupon/record",{data:JSON.stringify(data)},function(res){
			if(res.sc==0){
				var writeOffRecord="";
				...
				if (undefined != res.data.monthRecordInfo) {
					...
					for (var key in recordInfo) {
						...
						for (var i = 0; i < recordInfo[key].length; i++) {
							...
							var remianHtml="";
							if(undefined!=remian && remian>0 && res.data.couponBaseInfo.couponType != "9"){
								remianHtml='<br/>余额：'+remian/100;
							} else if (undefined!=remian && remian>0 && res.data.couponBaseInfo.couponType == "9") {
                                remianHtml='<br/>余额：'+remian+"天";
							}
							if(undefined!=amount && !isNaN(amount) && res.data.couponBaseInfo.couponType != "9"){
								if(amount>0){
									amount="+"+amount/100;
								}else {
									amount=amount/100;
							...
							}else if (undefined!=amount && !isNaN(amount) && res.data.couponBaseInfo.couponType == "9") {
                                if(amount>0){
									amount="+"+amount + "天";
								}else {
									amount=amount + "天";
...
function couponList(offData){
	$.post("/coupon/bms/applyHotelList",{data:JSON.stringify(offData)},function(offres){
		if(offres.sc==0){
			var tableHtml="";
			var thHtml="";
			...
			if(offres.data.length>0){
				for(var i=0;i<offres.data.length;i++){
					...
					//退卡功能加载数据-------------
                        // returnCard += "<li><span>"+ offres.data[i].couponBaseInfo.couponName +"</span><button class='returnCardBtn' couponCode="+ offres.data[i].couponCode +">退卡</button></li>";
					//----------------------------
					}
					if(i==0){
						thHtml="券资产： &nbsp;";
					...
					if(undefined!=remain && remain/1>=0 && offres.data[i].couponBaseInfo.couponType != "9"){
						console.log(2);
						remainHtml='<td style="min-width:160px;">可用额度：<span class="remian">'+remain/100+'</span></td>';
					} else if (undefined!=remain && remain/1>=0 && offres.data[i].couponBaseInfo.couponType == "9") {
                        remainHtml='<td style="min-width:160px;">可用额度：<span class="remian">'+remain+'天</span></td>';
					}
					if(undefined!=freeze){
...
$.ajax({
	...
	success:function(resp) {
	     if(resp.sc=='0'){
				...
				if(data.sysGenCouponCode!=undefined){
					var couponCode1=data.sysGenCouponCode.substring(0,data.sysGenCouponCode.length-6);
					var couponCode2=data.sysGenCouponCode.substring(data.sysGenCouponCode.length-6);
 ...
 function grantVipModal2RadioClick() {
	 ...
	 else if($('.grantVipModal2Radio').css('background-image')=='url("./images/gou2.jpg")'||'url("http://test.jiheclub.com/html/bms/pcBms/enquiry/images/gou2.jpg")'){
  ...
  function grantVipModal2AClick() {
    ...
    if($('.grantVipModal2Input:eq(1)').val()){
      ...
      data.grantList[0].memberProjectId=memberProjectId;
		...
		if(data.grantList[0].couponCode==undefined||data.grantList[0].couponCode.length==15){
   //判定是否点击单选框，若单选框选中则需判断卡号是否满足不重复；
	if($('.grantVipModal2Radio').css('background-image')=='url("./images/gou2.jpg")'||'url("http://test.jiheclub.com/html/bms/pcBms/enquiry/images/gou2.jpg")'){
...
//卡种类切换时，卡号变
 function changeCardId(){
	 ...
	 $.ajax({
		...
		success:function(resp) {
		if(resp.sc=='0'){
			var couponCode1=resp.data.cardCodeNext.substring(0,resp.data.cardCodeNext.length-6);
			var couponCode2=resp.data.cardCodeNext.substring(resp.data.cardCodeNext.length-6,resp.data.cardCodeNext.length);
