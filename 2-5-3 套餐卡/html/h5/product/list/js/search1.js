...
$(document).ready(function() {
	var url = h5orClient("/search/h5/default");
	var c = 1;
	var pageCount=0;//总页数
	var ajaxData=[];
	var dataPost = {"pagecnt": "5", "pageno": c,"defaultQuery":"2"};
	var pagecnt=5;//每页的个数
	...
	function addModel1(result,i){
		...
		var productName=(result[i].productBaseInfo.productName)?result[i].productBaseInfo.productName:'';
		var hotelCname=(productName!='')?'<div class="title">'+productName+'</div>':'';
		var tags="";
		...
		var typeHtml='';
		if(result[i].productBaseInfo.objectType==3){   //主题
			var topicName = (productName!='')?'<div class="topicTitle">'+productName+'</div>':'';
			var briefDesc = result[i].productBaseInfo.briefDesc?result[i].productBaseInfo.briefDesc:'';
			var briefDescHtml = (briefDesc!='')?'<span class="topicdesc">'+briefDesc+'</span>':'';
			typeHtml = '<div class="topic">' +
					topicName +
					briefDescHtml +
					'</div>';
		}else {
			typeHtml = '<div class="priceWrap">' +
