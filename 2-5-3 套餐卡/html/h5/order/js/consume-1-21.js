        ...
        $(".consume").hide();
            ...
            if(shouldPay!=0){
                ...
                if (ua.match(/MicroMessenger/i) == 'micromessenger') {
                    ...
                    $(".wxPay").hide();
                    $(".aliPay").find('em.iconfont').removeClass('iconfont-unconfirm2').addClass('iconfont-confirm2');
                }
            }else{
                $(".payChannel").hide();
  ...
  //  }
    $(".payBtn").click(
        function () {
            ...
            }else if($(".aliPay").find('em.iconfont').hasClass('iconfont-confirm2')){
                comfirmData={"id":id,"productId":productId,"amount": totalValue*100,"payments":payments,"quantity":1,"payChannel":"alipay_wap"};
            ...
            $.post('/order/h5/createandsubmit',{data:JSON.stringify(comfirmData)},function(data){
                if(data.sc==0){
                    var orderid=data.data.orderid,
                        hotelid=data.data.hotelId,
                        locationHref=window.location.href.split("/html")[0]+"/html/h5/order/orderDetailN.html?orderid="+orderid;
                    var payInfo={"payChannel":comfirmData.payChannel,"orderid":orderid};
                    if(payInfo.payChannel=="alipay_wap"){
                        payInfo.successurl=locationHref;
                        payInfo.cancelurl=locationHref;
                    }
                    $.post('/order/h5/pay',{data:JSON.stringify(payInfo)},function(data){
                        console.log(data);
                        if(data.sc==0){
                            if(data.data.payTicketInfo!=null){
                                if(data.data.payTicketInfo.channel=='wx_pub' || data.data.payTicketInfo.channel=='alipay_wap'){
