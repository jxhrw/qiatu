...
var cashPayment;//积分定价，资产提示用现金支付
var orderFieldList;//商品下单需填写的字段，1-姓名 2-手机号码 3-地址 4-有效证件 9-备注
...
$(document).ready(function(){
    ...
    $("#assets").on("click",".oneLine",function(){
        ...
        if("integralToRoom"==$(".choice",this).attr("id")){
            ...
            $(this).parents(".singleAsset").siblings().find("ul").hide();
            $(this).parents(".singleAsset").siblings().find(".arrowImg").removeClass("turnUp");
            $(this).parents(".singleAsset").find("ul").toggle();
            $(this).parents(".singleAsset").find(".arrowImg").toggleClass("turnUp");
            return false;
        }
        $.each($li,function(i){
    ...
    $("#createandsubmit").click(function(){
        ...
        if((roomPayAmount>0 && priceType=="0") || (cashPayment && priceType=="1")){
            var hasCashPay=0;
            var arr;
            if(priceType=="0"){
                arr=[{amount:roomPayAmount,paytype:0}];
            }else {
                if(cashPayment){
                    arr=cashPayment;
                }
            ...
            $.each(payments,function(i){
                if(payments[i].paytype=="0" || payments[i].paytype=="10"){
                    hasCashPay++;
                }
            });
            if(hasCashPay==0){
        ...
        var roomData;
        if("31"==objectType || "30"==objectType || "40"==objectType){ //30虚拟，31理财，40实物
            roomData = {'productid':GetParams.id,'quantity': roombook.quantity,'amount': productPrice,'payments': payments};
            if(priceType==1){
            ...
            var continueKey = 0;
            if(orderFieldList){
                $.each(orderFieldList,function(i){
                    switch(parseInt(orderFieldList[i])){
                        case 1:
                            if($("#virCusName").val()!=""){
                                roomData.customername= $("#virCusName").val();
                            }else {
                                continueKey++;
                            }
                            break;
                        case 2:
                            if($("#virCusPhone").val()!=""){
                                roomData.customermobile=$("#virCusPhone").val();
                            }else {
                                continueKey++;
                            }
                            break;
                        case 3:
                            if($("#virCusAddress").val()!=""){
                                roomData.customeraddress=$("#virCusAddress").val();
                            }else {
                                continueKey++;
                            }
                            break;
                        case 9:
                            roomData.comments= $("#virRemarks").val();
                            break;
                    }
                });
            }
            ...
            if($("#roomCusName").val()==""){
                errorPrompt("请填写联系人",2000);
                $("#createandsubmit").removeClass("no");
                return;
            }else if($("#roomCusPhone").val()==""){
                errorPrompt("请填写手机号",2000);
                $("#createandsubmit").removeClass("no");
                return;
            }
        ...
        if(GetParams.mc && "undefined"!=GetParams.mc){
            roomData.channel=GetParams.mc;
        }
        ...
        if(maxBuyQuantity!=-1){
            ...
            $.post(h5orClient(countList),{data:dataN},function(res) {
                ...
                if(showCyType=="0" || showCyType=="5"){
                    ...
                    if(showMaxPieces && showMaxPieces>0){
                        $("#virProPrice").html(showCyCode+Math.ceil(Math.round(showTotalPrice*num)/showMaxPieces)+showCyUnit);
                    }else{
    ...
    function roombookProductData(){
        ...
        $.post(h5orClient(roomProUrl),{data:JSON.stringify(productId)},function(roomProRes){
            if(roomProRes.sc==0){
                ...
                showMaxPieces=roomProRes.data.productPrice.showMaxPieces;//1张券换多份商品
                orderFieldList=roomProRes.data.orderFieldList;
                ...
                var productPriceInfo={
                    ...
                    showMaxPieces:roomProRes.data.productPrice.showMaxPieces
                };
                productInfo={
                ...
                //虚拟产品显示库存
                if(roomProRes.data.tolAmount && roomProRes.data.tolAmount>0){
                    $("#virProQuantity").parents(".oneLine").css({"height":"4.23rem"}).append('<div class="thisTolAmount">库存： &nbsp;'+roomProRes.data.tolAmount+'</div>');
                    $(".thisTolAmount").css({
                        "width": "100%",
                        "float": "right",
                        "line-height": "0.8rem",
                ...
                if("31"==objectType || "30"==objectType || "40"==objectType){
                    ...
                    if(buyPointsId==GetParams.id && undefined!=GetParams.needpoints){
                        roombook.quantity=Math.ceil(GetParams.needpoints/100);
                    }
                    if("31"==objectType){
                    ...
                    if(orderFieldList){
                        $.each(orderFieldList,function(i){
                            switch(parseInt(orderFieldList[i])){
                                case 1:
                                    $("#virCusName").parents(".oneLine").show();
                                    break;
                                case 2:
                                    $("#virCusPhone").parents(".oneLine").show();
                                    break;
                                case 3:
                                    $("#virCusAddress").parents(".oneLine").show();
                                    break;
                                case 9:
                                    $("#virRemarks").parents(".oneLine").show();
                                    break;
                            }
                        });
                    }
                    ...
                    if(showCyType=="0" || showCyType=="5"){
                        ...
                        if(showCyType=="4" && showMaxPieces && showMaxPieces>0){
                            roombook.quantity=showMaxPieces;
                            $("#virProUnit").html(showMaxPieces+roomProRes.data.piecesUnit+'/'+Math.round(showTotalPrice)+showCyUnit+' '+showCyCode);
                            $("#virProPrice").html(showCyCode+Math.ceil(Math.round(showTotalPrice*roombook.quantity)/showMaxPieces)+showCyUnit);
                        }else {
                    ...
                    productPrice=floatFixed2(unitPrice*roombook.quantity);
                    productPiontPrice=Math.round(unitPricePoint*roombook.quantity);
                    $("#virProQuantity").val(roombook.quantity);
                    ...
                    $(".singleNote").show().find("#retire").html(roomProRes.data.cancelPolicy?'<span>退改政策</span>'+roomProRes.data.cancelPolicy:'');
        ...
        $.each(couponDisplayList,function(i){
            if(couponDisplayList[i].isSelected){
                ...
                if(showCyType=="0"){
                    ...
                    if(couponDisplayList[i].couponBaseInfo.couponType=="3"){
                        dikou=((couponDisplayList[i].paidFaceValue)?couponDisplayList[i].paidFaceValue/100:0);
                    }
                }else {
                    if(showCyType=="1"){
                        dikou=showCyCode+((couponDisplayList[i].costPoints)?couponDisplayList[i].costPoints:0)+showCyUnit;
                    }
                    else if(showCyType=="4"){
                        action="扣减";
                        deductionDesc="";
                        dikou=((couponDisplayList[i].paidFaceValue)?couponDisplayList[i].paidFaceValue:((couponDisplayList[i].paidPieces)?couponDisplayList[i].paidPieces:0))+showCyUnit;
                    }
        ...
        $.post(h5orClient(assetsUrl),{data:JSON.stringify(assetsData)},function(assetsRes){
            if(assetsRes.sc==0){
                ...
                if(firstLoad=="first"){
                    ...
                    if(assetsRes.data.membershipCardInfo){
                        ...
                        $.each(membershipCardList,function(i){
                            ...
                            var effectiveTime=parseInt(membershipCardList[i].effectiveTime);
                            var expireTime=parseInt(membershipCardList[i].expireTime);
                            var remain,discount;
                            var discounthtml='';
                        ...
                        $("#membershipCardInfo").html(membershipCardUl).show().parents(".singleAsset").find(".arrowImg").addClass("turnUp");
                    }
                    else {
                        $("#membershipCardInfo").parents(".singleAsset").remove();
                    }
                    //消费金
                    if(assetsRes.data.cashCouponInfo){
                        ...
                        $.each(cashCouponList,function(i){
                            ...
                            var effectiveTime=parseInt(cashCouponList[i].effectiveTime);
                            var expireTime=parseInt(cashCouponList[i].expireTime);
                        ...
                        $("#cashCouponInfo").parents(".singleAsset").remove();
                    }
                    //房券
                    if(assetsRes.data.roomCouponInfo){
                        ...
                        $.each(roomCouponList,function(i){
                            ...
                            var effectiveTime=parseInt(roomCouponList[i].effectiveTime);
                            var expireTime=parseInt(roomCouponList[i].expireTime);
                        ...
                        $("#roomCouponInfo").parents(".singleAsset").remove();
                    }
                    //折扣券
                    if(assetsRes.data.discountCouponInfo){
                        ...
                        $.each(discountCouponList,function(i){
                            ...
                            var effectiveTime=parseInt(discountCouponList[i].effectiveTime);
                            var expireTime=parseInt(discountCouponList[i].expireTime);
                        ...
                        $("#discountCouponInfo").parents(".singleAsset").remove();
                    }
                    //红包
                    if(assetsRes.data.redPacketInfo){
                        ...
                        $.each(redPacketList,function(i){
                            ...
                            var effectiveTime=parseInt(redPacketList[i].effectiveTime);
                            var expireTime=parseInt(redPacketList[i].expireTime);
                        ...
                        $("#redPacketInfo").parents(".singleAsset").remove();
                    }
                    //积分兑房
                    if(assetsRes.data.roomPointsInfo){
                        ...
                        $("#roomPointsInfo").parents(".singleAsset").remove();
                    }
                    //交换资产
                    if(assetsRes.data.jiheXCouponInfo){
                        ...
                        $.each(jiheXCouponList,function(i){
                            ...
                            var effectiveTime=parseInt(jiheXCouponList[i].effectiveTime);
                            var expireTime=parseInt(jiheXCouponList[i].expireTime);
                ...
                if(assetsRes.data.membershipCardInfo && assetsRes.data.membershipCardInfo.isSelected){
                    ...
                    deductible($("#mbCardsInfo"),assetsRes.data.membershipCardInfo.selectedCouponValue);
                }else {
                    $("#mbCards").removeClass("choosed");
                    assetsHide("#membershipCardInfo");
                    if(assetsRes.data.membershipCardInfo){
                        available($("#mbCardsInfo"),assetsRes.data.membershipCardInfo.availableCouponPieces);
                    }
                }
                //消费金展示
                if(assetsRes.data.cashCouponInfo && assetsRes.data.cashCouponInfo.isSelected){
                    ...
                    deductible($("#cashCouponsInfo"),assetsRes.data.cashCouponInfo.selectedCouponValue);
                }else {
                    $("#cashCoupons").removeClass("choosed");
                    assetsHide("#cashCouponInfo");
                    if(assetsRes.data.cashCouponInfo){
                        available($("#cashCouponsInfo"),assetsRes.data.cashCouponInfo.availableCouponPieces);
                    }
                }
                //房券展示
                if(assetsRes.data.roomCouponInfo && assetsRes.data.roomCouponInfo.isSelected){
                    ...
                    deductible($("#roomCouponsInfo"),assetsRes.data.roomCouponInfo.selectedCouponValue);
                }else {
                    $("#roomCoupons").removeClass("choosed");
                    assetsHide("#roomCouponInfo");
                    if(assetsRes.data.roomCouponInfo){
                        available($("#roomCouponsInfo"),assetsRes.data.roomCouponInfo.availableCouponPieces);
                    }
                }
                //折扣券展示
                if(assetsRes.data.discountCouponInfo && assetsRes.data.discountCouponInfo.isSelected){
                    ...
                    deductible($("#discCouponsInfo"),assetsRes.data.discountCouponInfo.selectedCouponValue);
                }else {
                    $("#discCoupons").removeClass("choosed");
                    assetsHide("#discountCouponInfo");
                    if(assetsRes.data.discountCouponInfo){
                        available($("#discCouponsInfo"),assetsRes.data.discountCouponInfo.availableCouponPieces);
                    }
                }
                //红包展示
                if(assetsRes.data.redPacketInfo && assetsRes.data.redPacketInfo.isSelected){
                    ...
                    deductible($("#envelopesInfo"),assetsRes.data.redPacketInfo.selectedCouponValue);
                }else {
                    $("#envelopes").removeClass("choosed");
                    assetsHide("#redPacketInfo");
                    if(assetsRes.data.redPacketInfo){
                        available($("#envelopesInfo"),assetsRes.data.redPacketInfo.availableCouponPieces);
                    }
                }
                //积分兑房展示
                ...
                if(assetsRes.data.pointsFixedInfo){
                    ...
                    if(assetsRes.data.pointsFixedInfo.paymentInfoList){
                        ...
                        if(pointsFixedDesc.indexOf("应付为0")>-1){
                            ...
                            $("#buyPoints").show().attr("href","../product/detail/virtualGoods.html?mc=zqjf&id="+buyPointsId+"&needpoints="+productPiontPrice);
                        }
                    }
                    $("#myPoints").html(pointsFixedDesc);
                }
                //积分定价是否提交现金
                cashPayment=assetsRes.data.cashPayment;
                if(assetsRes.data.cashPayment){
                    cashToPoint=cashPayment.points?cashPayment.points:0;
                    $("#roomPayAmount").html("￥"+(cashPayment.faceValue?cashPayment.faceValue:0)/100);
                }

                if(priceType==1){
                    roomPayAmount=productPiontPrice-membershipCardAmount-couponAmount-roomCouponAmount-discountCouponAmount-redPacketAmount-roomPointsAmount-jiheXCouponAmount-cashPointsAmount-payPointAmounts-cashToPoint;
                    if(roomPayAmount>0){
                        $("#createandsubmit").addClass("unSubmit");
                    }
                    else {
                    ...
                    if(roomPayAmount>0 && $.inArray("0", payTypes)!=-1 && (!payChannelList || payChannelList.length==0 || (payChannelList.length==1 && payChannelList[0]==""))){
                        $("#createandsubmit").addClass("unSubmit");
                    }
    ...
    //本店资产大项抵扣
    function deductible(dom,value){
        if(value){
            if(showCyType=="0" || showCyType=="5"){
                value=value/100;
            }
            dom.html('-'+showCyCode+value+showCyUnit).addClass("paleRed");
        }
    }
    //本店资产几张可用
    function available(dom,value){
        if(value){
            dom.html(value+"张可用").removeClass("paleRed");
        }
    }
